...
//   }
// }
void MessageLog::save(cereal::JSONOutputArchive& archive) {
  archive(messages_.size());
  for (Message message : messages_) {
    // archive(message.text_.c_str());
    archive(message.text_);
    archive(message.colour_.r);
    archive(message.colour_.g);
    archive(message.colour_.b);
    archive(message.count_);
  }
}
void MessageLog::load(cereal::JSONInputArchive& archive) {
  int nbMessages;
  archive(nbMessages);
  while (nbMessages > 0) {
    // char* text;
    ...
    messages_.emplace_back(
        text, tcod::ColorRGB{(uint8_t)r, (uint8_t)g, (uint8_t)b}, count);
  }
}
